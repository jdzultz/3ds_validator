...
class WebViewLibrary(context: Context): BaseViewModel(CoroutineContextProviderImp())
{
    ...
    private val gatewayRemoteImp = GatewayRemoteImp(
        ServiceFactory.create(true, "https://qa.apimop.xcaret.com/api/Payment/"),
    ...
    fun getCallGateway(params: GatewayPayload) {
    ...
    fun launchFormOne (cardNumber: String, token: String, callback: ((data: PostMessage?) -> Unit)? = null) {
        dialogGetSessionId.show(cardNumber, token) {
            response ->
            if (callback != null) {
                callback(response)
            }
    ...
    fun launchFormTwo (md: String, jwt: String, callback: (() -> Unit)? = null) {
        dialogGetView3DS.show(md, jwt){
            if (callback != null) {
                callback()
            }
        }
    ...
    private suspend fun loadGateway(params: GatewayPayload) {
